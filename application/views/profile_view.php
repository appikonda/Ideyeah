...
<br/>
<?php 
   echo 'Name: '.$user_info[0]->first_name.' '.$user_info[0]->last_name.'<br/>';
   echo 'Bio: '.$user_info[0]->bio.'<br/>';
   echo 'URL: <a href="'.$user_info[0]->url.'">'.$user_info[0]->url.'</a><br/><br/>';
?>
<fieldset>
<legend><h2>My Ideas</h2></legend>   
<?php 
   $count = 1;
   if(isset($ideas)) {
...
?>
<fieldset>
<legend><h2>My Contribution</h2></legend>
<?php 
   $count = 1;
   if(isset($contribution)) {
      foreach($contribution as $comment) {      
         echo '<br/><b>'.$count.'. Idea Title:</b> <u><a href="'.base_url().'idea/index/'.$comment->idea_id.'">'.$comment->idea_title.'</a></u> (Posted on: '.$comment->date.')<br/><b>Comment:</b> '.$comment->comment_content.'<br/>';
         $count++;
      }
   }
?>
